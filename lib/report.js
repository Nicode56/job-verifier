...
async function generateReport(input) {
  ...
  const [companySite, identity] = await Promise.all([
  ...
  // versus if we have no other signal about it at all.
  let reachableResult = companySite.result;
  if (companySite.possibleBotBlock && identity.ageDays != null && identity.ageDays >= 365) {
    const years = (identity.ageDays / 365).toFixed(1);
    reachableResult = check('company-reachable', 'Company website reachable', WARN,
      `${domain} responded with HTTP ${companySite.httpStatus} rather than a normal success response, so its page content couldn't be read directly. That's inconclusive by itself — but this domain has been registered for approximately ${years} years, consistent with an established, active organization, which makes automated-request blocking (bot protection) a more likely explanation than the domain being fake or inactive.`);
  }
  ...
  sections.push({
    name: 'Company',
    checks: [reachableResult, companyNameCheck, identity.result, careers.result]
  });
