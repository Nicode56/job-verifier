...
async function mxRecordCheck(email) {
  ...
  try {
    const records = await withDnsTimeout(dns.resolveMx(emailDomain));
    ...
    if (err.code === 'DNS_TIMEOUT') {
      return check('recruiter-mx', 'Recruiter domain can receive mail', WARN,
        `Mail configuration for ${emailDomain} could not be checked in time (the DNS lookup timed out). This is a lookup limitation, not a confirmed problem with the domain.`);
    }
    return check('recruiter-mx', 'Recruiter domain can receive mail', FAIL,
...
async function mailAuthCheck(email) {
  ...
  try {
    const [spfRecords, dmarcRecords] = await Promise.all([
      withDnsTimeout(dns.resolveTxt(emailDomain)).catch(() => []),
      withDnsTimeout(dns.resolveTxt(`_dmarc.${emailDomain}`)).catch(() => [])
    ]);
