...
async function certificateCheck(domain) {
  ...
  if (!cert.present) {
    return {
      result: check('company-tls', 'Website has a valid TLS certificate', WARN,
        `A direct TLS connection to ${domain} on port 443 could not be established (${cert.reason}), so its certificate could not be independently checked.`),
      authorized: false
    };
  }
  if (cert.authorized) {
    return {
      result: check('company-tls', 'Website has a valid TLS certificate', PASS,
        `${domain} presents a TLS certificate issued by ${cert.issuer}, valid until ${cert.validTo}, that is trusted and matches this domain — confirming a real, properly configured server is live for this domain, independent of whether its page content could be read.`),
      authorized: true
    };
  }
  return {
    result: check('company-tls', 'Website has a valid TLS certificate', WARN,
      `${domain} presents a TLS certificate, but it did not validate (${cert.authorizationError}). That can indicate misconfiguration rather than anything about the company itself, but it isn't the clean confirmation a fully valid certificate would be.`),
    authorized: false
  };
}
...
async function domainIdentityCheck(domain) {
  try {
    ...
    if (!res.ok) {
      return {
        result: check('company-identity', 'Company identity established', WARN,
    ...
    if (ageDays < 90) {
      return {
        result: check('company-identity', 'Company identity established', FAIL,
    ...
    return {
      result: check('company-identity', 'Company identity established', PASS,
...
async function careersPageCheck(domain, homepageBody) {
  ...
  const urls = candidates.map(path => (path.startsWith('http') ? path : `https://${domain}${path}`));

  // Probed concurrently: each probe is already capped by fetchWithTimeout,
  // so doing this sequentially would let worst-case latency multiply by the
  // number of candidate paths (up to a minute) instead of staying bounded
  // by a single timeout.
  ...
  if (found) {
    return {
      result: check('careers-page', 'Official careers page found', PASS,
        `${found.finalUrl} responded successfully and appears to be the company's own careers/jobs page.`),
      careersUrl: found.finalUrl,
      careersBody: found.body
    };
  }
...
module.exports = { websiteReachableCheck, companyNameOnSiteCheck, domainIdentityCheck, careersPageCheck, certificateCheck };
